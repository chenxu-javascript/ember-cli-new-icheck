...
export default Checkbox.extend({
	...
	didInsertElement() {
		...
		assert("iCheck has to exist on Ember.$.fn.iCheck", $.fn.iCheck);

		let icheck = this.$().iCheck({
      // base class added to customized checkboxes
		...
		icheck.on('ifToggled', (ev) => {
			this.set('checked', ev.target.checked);
		});

		this.set('_icheck', icheck);
	},

  _checkedChanged: observer('checked', function() {
    let checked = this.get('checked') ? 'check' : 'uncheck';
